...
class MinCostRegNet(nn.Module):
    def __init__(self, in_channels, norm_act=nn.BatchNorm3d):
        ...
        self.conv9 = ConvBnReLU3D(32, 16, norm_act=norm_act)
        self.conv11 = ConvBnReLU3D(16, 8, norm_act=norm_act)
    ...
    def forward(self, x):
        ...
        conv4 = self.conv4(self.conv3(conv2))
        x = conv4
        x = conv2 + self.conv9(F.interpolate(x, scale_factor=2., align_corners=True, mode='trilinear'))
        del conv2
        x = conv0 + self.conv11(F.interpolate(x, scale_factor=2., align_corners=True, mode='trilinear'))
        del conv0
        feat = self.feat_conv(x)
        depth = self.depth_conv(x)
        return feat, depth.squeeze(1)
